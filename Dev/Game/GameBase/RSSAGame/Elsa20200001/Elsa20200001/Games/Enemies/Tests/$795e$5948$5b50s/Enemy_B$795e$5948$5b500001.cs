...
namespace Charlotte.Games.Enemies.Tests.神奈子s
{
	public class Enemy_B神奈子0001 : Enemy
	{
		...
		protected override IEnumerable<bool> E_Draw()
		{
			...
			for (int frame = 0; ; frame++)
			{
				if (!_hitBack())
				{
					...
					DDDraw.DrawBegin(Ground.I.Picture2.Enemy_B神奈子[0], this.X, this.Y);
		...
		private IEnumerable<bool> E_HitBack()
		{
			...
			{
				double xBuru = DDUtils.Random.Double() * 10.0;
				double yBuru = DDUtils.Random.Double() * 10.0;

				bool facingLeft = Game.I.Player.X < this.X;

				DDDraw.DrawBegin(Ground.I.Picture2.Enemy_B神奈子[12], this.X + xBuru, this.Y + yBuru);
				DDDraw.DrawSlide(20.0, -10.0);
				DDDraw.DrawZoom_X(facingLeft ? 1 : -1);
				DDDraw.DrawEnd();
