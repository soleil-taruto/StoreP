	...
	public class Scenario
	{
		...
		public Scenario(string name)
		{
			...
			// シナリオデータに共通部を差し込み
			{
				...
				lines = SCommon.InsertArray(lines, index + 1, new string[] { "#Include/Startup" });
			}
			...
			for (int index = 0; index < lines.Length; index++)
			{
				...
				if (line[0] == '#') // ? 外部ファイル参照
				{
					...
					lines = SCommon.RemoveArray(lines, index, 1);
					lines = SCommon.InsertArray(lines, index, includeLines);
				}
			}
